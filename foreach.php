...
<body>
  <p>
    ...
    // 配列$user‗namesの要素を1つずつ順番に出力する
    foreach ($user_names as $user_name) {
      echo $user_name . '<br>';
    }

      // 配列$user‗namesのインデックスと値を1つずつ順番に出力する
  ...
  <p>
    ...
    // 連想配列$personal‗dataのキーと値を1つずつ順番に出力する
    foreach ($personal_data as $key => $value) {
      echo "{$key}は{$value}です。<br>";
    }

    // 連想配列$personal‗dataの値を１つずつ順番に出力する
    foreach ($personal_data as $key => $value) {
      echo $value . '<br>';
    }
    ?>
  ...
  <p>
    ...
    foreach ($user_names as $user_name) {
      ...
      // 変数$user‗nameと変数$targetの値が一致すれば、break文で繰り返し処理を強制終了する
      if ($user_name === $target) {
